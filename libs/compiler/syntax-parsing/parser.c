...
#if 0
	#define DEBUG_PRODUCTION(x) do { printf(x); printf("\n"); } while(0);
#else
...
static int el_parse_parameter_list(struct el_token_stream * token_stream, struct el_ast_parameter_list * parameter_list);
static int el_parse_parameters(struct el_token_stream * token_stream, struct el_ast_parameter_list * parameter_list);
static int el_parse_parameter(struct el_token_stream * token_stream, struct el_ast_var_decl * var_decl);
...
static int el_parse_assignment(struct el_token_stream * token_stream, struct el_ast_expression * expression);
static int el_parse_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression);
static int el_parse_function_call(struct el_token_stream * token_stream, struct el_ast_expression_list * expression_list);
static int el_parse_arguments(struct el_token_stream * token_stream, struct el_ast_expression_list * expression_list);
...
static int el_parse_complex_identifier(struct el_token_stream * token_stream, struct el_ast_expression * expression);
static int el_parse_complex_identifier_inner(struct el_token_stream * token_stream, struct el_ast_expression * expression);

static int el_parse_boolean_or_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression);
static int el_parse_boolean_and_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression);
static int el_parse_add_or_sub_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression);
static int el_parse_mult_or_div_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression);
static int el_parse_factor_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression);

static int el_convert_to_binary_op(struct el_ast_expression * expression, int type);
static int el_new_expr_list(struct el_ast_expression * expression, int type);
...
static int el_parse_function(struct el_token_stream * token_stream, struct el_ast_statement_list * parent)
{
	...
	err = err || el_parse_parameter_list(token_stream, &function_definition->parameter_list);
...
static int el_parse_parameter_list(struct el_token_stream * token_stream, struct el_ast_parameter_list * parameter_list)
{
	...
	int err = 0;
	parameter_list->parameters = fmalloc(sizeof(struct el_ast_var_decl) * MAX_NUM_PARAMS_PER_PARAM_LIST);
	...
	err = err || el_parse_parameters(token_stream, parameter_list);
...
static int el_parse_parameters(struct el_token_stream * token_stream, struct el_ast_parameter_list * parameter_list)
{
	...
	err = err || el_parse_parameter(token_stream, &parameter_list->parameters[parameter_list->num_parameters++]);
...
static int el_parse_parameter(struct el_token_stream * token_stream, struct el_ast_var_decl * var_decl)
{
	...
	err = err || el_match_token(token_stream, el_IDENTIFIER);
	err = err || el_parse_optional_type(token_stream, &var_decl->type);
	return err;
}
...
static int el_parse_code_block_statement(struct el_token_stream * token_stream, struct el_ast_statement_list * list)
{
	...
	else if(el_is_lookahead(token_stream, el_RET_KEYWORD))
	{
		...
		err = err || el_parse_expr(token_stream, &list->statements[statement_index].return_statement.expression);
	}
	else
	{
		int statement_index = list->num_statements++;
		list->statements[statement_index].type = el_AST_NODE_EXPRESSION;
		err = err || el_parse_complex_identifier(token_stream, &list->statements[statement_index].expression);
		if(el_is_lookahead(token_stream, el_ASSIGN_OPERATOR))
		{
			...
			err = err || el_parse_assignment(token_stream, &list->statements[statement_index].assignment.rhs);
		}
	}
	return err;
}
...
static int el_parse_for_statement(struct el_token_stream * token_stream, struct el_ast_statement_list * parent)
{
	...
	err = err || el_parse_complex_identifier(token_stream, &for_statement->range);
...
static int el_parse_if_statement(struct el_token_stream * token_stream, struct el_ast_statement_list * parent)
{
	...
	err = err || el_parse_expr(token_stream, &if_statement->expression);
...
static int el_parse_elif_statements(struct el_token_stream * token_stream, struct el_ast_if_statement * parent)
{
	...
	err = err || el_parse_expr(token_stream, &elif_statement->expression);
...
static int el_parse_assignment(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	err = err || el_parse_expr(token_stream, expression);
	return err;
}
...
static int el_parse_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	int err = 0;
	err = err || el_parse_boolean_or_expr(token_stream, expression);
	if(
		...
		el_is_lookahead(token_stream, el_GEQUALS_COMPARATOR))
	{
		...
		{
			err = err || el_convert_to_binary_op(expression, el_AST_EXPR_EQUALS);
			err = err || el_match_token(token_stream, el_EQUALS_COMPARATOR);
			err = err || el_parse_expr(token_stream, expression->binary_op.rhs);
		}
		else if(el_is_lookahead(token_stream, el_GREATER_THAN_COMPARATOR))
		{
			err = err || el_convert_to_binary_op(expression, el_AST_EXPR_GREATER_THAN);
...
static int el_parse_function_call(struct el_token_stream * token_stream, struct el_ast_expression_list * expression_list)
{
	...
	int err = 0;
	err = err || el_match_token(token_stream, el_PARENTHESIS_OPEN);
	err = err || el_parse_arguments(token_stream, expression_list);
	err = err || el_match_token(token_stream, el_PARENTHESIS_CLOSE);
	return err;
}
...
static int el_parse_arguments(struct el_token_stream * token_stream, struct el_ast_expression_list * expression_list)
{
	...
	err = err || el_parse_expr(token_stream, &expression_list->expressions[expression_list->num_expressions++]);
	if(el_is_lookahead(token_stream, el_COMMA_SEPARATOR))
	{
		// NOTE - This allows a trailing comma before the closing bracket
		err = err || el_match_token(token_stream, el_COMMA_SEPARATOR);
...
static int el_parse_complex_identifier(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	expression->identifier = el_string_new(el_lookahead(token_stream)->source, -1);
	err = err || el_match_token(token_stream, el_IDENTIFIER);
	err = err || el_parse_complex_identifier_inner(token_stream, expression);
	return err;
}
...
static int el_parse_complex_identifier_inner(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_SLICE_INDEX);
		err = err || el_match_token(token_stream, el_SLICE_START);
		err = err || el_parse_expr(token_stream, expression->binary_op.rhs);
		err = err || el_match_token(token_stream, el_SLICE_END);
	}
	...
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_FUNCTION_CALL);
		err = err || el_new_expr_list(expression->binary_op.rhs, el_AST_EXPR_ARGUMENTS);
		err = err || el_parse_function_call(token_stream, expression->binary_op.rhs->expression_list);
	}
	else if(el_is_lookahead(token_stream, el_DOT_OPERATOR))
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_DOT);
		err = err || el_match_token(token_stream, el_DOT_OPERATOR);
		expression->binary_op.rhs->type = el_AST_EXPR_IDENTIFIER;
		expression->binary_op.rhs->identifier = el_string_new(el_lookahead(token_stream)->source, -1);
	...
	err = err || el_parse_complex_identifier_inner(token_stream, expression->binary_op.rhs);
	return err;
}
...
static int el_parse_boolean_or_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	int err = 0;
	err = err || el_parse_boolean_and_expr(token_stream, expression);
	if(el_is_lookahead(token_stream, el_BOOLEAN_OR))
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_BOOLEAN_OR);
		err = err || el_match_token(token_stream, el_BOOLEAN_OR);
		err = err || el_parse_boolean_or_expr(token_stream, expression->binary_op.rhs);
	}
	return err;
}
...
static int el_parse_boolean_and_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	int err = 0;
	err = err || el_parse_add_or_sub_expr(token_stream, expression);
	if(el_is_lookahead(token_stream, el_BOOLEAN_AND))
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_BOOLEAN_AND);
		err = err || el_match_token(token_stream, el_BOOLEAN_AND);
		err = err || el_parse_boolean_and_expr(token_stream, expression->binary_op.rhs);
	}
	return err;
}
...
static int el_parse_add_or_sub_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	int err = 0;
	err = err || el_parse_mult_or_div_expr(token_stream, expression);
	if(el_is_lookahead(token_stream, el_ADD_OPERATOR))
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_ADD);
		err = err || el_match_token(token_stream, el_ADD_OPERATOR);
		err = err || el_parse_add_or_sub_expr(token_stream, expression->binary_op.rhs);
	}
	else if(el_is_lookahead(token_stream, el_SUBTRACT_OPERATOR))
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_SUB);
...
static int el_parse_mult_or_div_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	int err = 0;
	err = err || el_parse_factor_expr(token_stream, expression);
	if(el_is_lookahead(token_stream, el_MULTIPLY_OPERATOR))
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_MUL);
		err = err || el_match_token(token_stream, el_MULTIPLY_OPERATOR);
		err = err || el_parse_mult_or_div_expr(token_stream, expression->binary_op.rhs);
	}
	else if(el_is_lookahead(token_stream, el_DIVIDE_OPERATOR))
	{
		err = err || el_convert_to_binary_op(expression, el_AST_EXPR_DIV);
...
static int el_parse_factor_expr(struct el_token_stream * token_stream, struct el_ast_expression * expression)
{
	...
	if(el_is_lookahead(token_stream, el_NUMBER_LITERAL))
	{
		...
		expression->number_literal = el_string_new(el_lookahead(token_stream)->source, -1);
	...
	else if(el_is_lookahead(token_stream, el_STRING_LITERAL))
	{
		...
		expression->string_literal = el_string_new(el_lookahead(token_stream)->source, -1);
		err = err || el_match_token(token_stream, el_STRING_LITERAL);
	}
	else if(el_is_lookahead(token_stream, el_IDENTIFIER))
	{
		err = err || el_parse_complex_identifier(token_stream, expression);
	}
	else if(el_is_lookahead(token_stream, el_PARENTHESIS_OPEN))
	{
		err = err || el_match_token(token_stream, el_PARENTHESIS_OPEN);
	...
	{
		err = err || el_new_expr_list(expression, el_AST_EXPR_SLICE_LITERAL);
		err = err || el_match_token(token_stream, el_SLICE_START);
		err = err || el_parse_arguments(token_stream, expression->expression_list);
		err = err || el_match_token(token_stream, el_SLICE_END);
	}
	else
	{
...
int el_convert_to_binary_op(struct el_ast_expression * expression, int type)
{
	struct el_ast_expression lhs = *expression;
	expression->type = type;
	expression->binary_op.lhs = fmalloc(sizeof *expression);
	expression->binary_op.rhs = fmalloc(sizeof *expression);
...
int el_new_expr_list(struct el_ast_expression * expression, int type)
{
	expression->type = type;
	expression->expression_list = fmalloc(sizeof(struct el_ast_expression_list));
	if(!expression->expression_list)
	{
		return el_ALLOCATION_ERROR;
	}
	expression->expression_list->expressions = fmalloc(sizeof(struct el_ast_expression) * MAX_NUM_EXPRS_PER_EXPR_LIST);
